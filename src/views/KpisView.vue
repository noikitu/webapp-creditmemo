<script setup lang="ts">
  ...
  import { api, type KpiLineage } from '@/api';
  import { cn } from '@/lib/utils';

  const kpis = ref<KpiLineage[]>([]);
  const loading = ref(true);
  const selected = ref<KpiLineage | null>(null);
...
<template>
  <div class="max-w-5xl mx-auto px-8 py-10">
    ...
    <p class="text-muted-foreground mb-8">
      Select a KPI to see its lineage: the formula, the metrics it uses, and the documents they come from.
    </p>
    ...
    <div v-if="kpis.length" class="grid grid-cols-3 gap-6">
        ...
        <div v-for="c in categories" :key="c">
          ...
          <div class="space-y-1">
            <button v-for="k in inCategory(c)" :key="k.kpi" type="button"
              @click="selected = k"
              :class="cn('w-full text-left rounded-md px-3 py-2 text-sm transition-colors',
      ...
      <!-- Right: lineage of the selected KPI -->
      <div class="col-span-2">
        <Card v-if="selected">
          <CardContent class="pt-5 space-y-5">
            <div>
              <div class="text-lg font-semibold">{{ selected.kpi }}</div>
            ...
            <div>
              ...
              <div v-if="selected.metrics.length" class="space-y-2">
                <div v-for="m in selected.metrics" :key="m.metric"
              ...
              <div v-else class="text-sm text-muted-foreground">
                No metric detected in the formula.
              </div>
            </div>
          </CardContent>
        </Card>
