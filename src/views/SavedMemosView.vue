...
<template>
  <div class="max-w-5xl mx-auto px-8 py-10">
    ...
    <div class="grid gap-4 sm:grid-cols-2">
      <Card v-for="m in memos" :key="m.id" class="group">
        <CardContent class="pt-6">
          <div class="flex items-start justify-between gap-3">
            <button type="button" class="flex items-start gap-3 text-left min-w-0" @click="open(m.id)">
              ...
              <span class="min-w-0">
                <span class="block font-medium truncate">{{ m.title || 'Untitled memo' }}</span>
                <span class="block text-sm text-muted-foreground">
                  {{ m.blocks.length }} section{{ m.blocks.length > 1 ? 's' : '' }}
                </span>
              </span>
            </button>
            <Button variant="ghost" size="icon" class="h-8 w-8 text-destructive opacity-0 group-hover:opacity-100"
              @click="remove(m.id, m.title)"><Trash2 class="h-4 w-4" /></Button>
          </div>
          <Button variant="ghost" size="sm" class="mt-3 w-full justify-between" @click="open(m.id)">
